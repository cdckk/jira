...
//2.在函数里，改变传入的对象本身是不好的
//不要污染传入的对象
const isFalsy = (value: any) => value === 0 ? false : !value
export const cleanObject = (object: any) => {
  const result = {...object}
  Object.keys(result).forEach(key => {
    // 3.要弄清楚key和key值（value）
    // 不要判断key，key只是字符串 没意义
    const value = result[key]
    if(isFalsy(value)) {
      delete result[key]
    }
...
export const useDocumentTitle = (title: string, onKeepMount: boolean) => {
  // 第一次渲染时是react默认的文档标题
...
/**
 * 返回组件的挂载状态, 没有挂载或已卸载返回false，反之返回true
 */
export const useMountedRef = () => {
  const mountedRef = useRef(false)
